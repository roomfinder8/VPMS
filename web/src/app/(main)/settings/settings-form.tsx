    ...
    <form onSubmit={submit} className="flex flex-col gap-6">
      <section className="flex flex-col gap-4 rounded-xl border border-line bg-raised p-4">
        <div>
          ...
          <p className="text-sm text-ink-faint">
            One address per line. This is the only email the system sends — it goes
            to whoever checks the day&rsquo;s numbers, who then writes to the manager
            from their own mailbox.
          </p>
        </div>

        <Field label="Report goes to">
          <textarea
            rows={3}
      ...
      <section className="flex flex-col gap-4 rounded-xl border border-line bg-raised p-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Send the report at">
            <input
              type="time"
        ...
        <label className="flex items-start gap-3">
          ...
          <span>
            ...
            <span className="block text-xs text-ink-faint">
              Before sending, set the exit time to check-in plus the free hours that
              were stamped, and flag it as estimated. &ldquo;Free all day&rdquo; has no
              hour count, so those are flagged as having no exit time instead.
            </span>
          </span>
        </label>
      </section>
